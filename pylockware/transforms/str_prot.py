...
class StringProtectionTransformer(ast.NodeTransformer):
    ...
    def _protect_string(self, node):
        """Protect a string node by encoding it and returning an inline decode call."""
        ...
        if any(original_string.strip().startswith(pattern) for pattern in suspicious_patterns):
            return node
        ...
        if (
                ...
                and any(spec in original_string for spec in ["%s", "%d", "%f", "{:", "format"])
        ):
            return node
...
def {transformer.fstring_func_name}(*args):
    ...
    for arg in args:
        ...
        if isinstance(arg, str):
            ...
            # This is an expression result (int, object, etc.)
            result += str(arg)
    return result
...
def protect_strings_in_directory(directory):
    ...
    for py_file in Path(directory).rglob("*.py"):
        if protect_strings_in_file(py_file):
            ...
            # Count protected strings by checking for the decode function calls
            with open(py_file, encoding="utf-8") as f:
                content = f.read()
                total_strings += content.count("(")  # Simple approximation
