    ...
    def build_global_replacements(self, directory: Path):
        ...
        for name in sorted(all_names):  # Sort for consistent ordering
            ...
            if name not in self.remap_map and not name.startswith('_imp_'):
                if name == 'main':
                    print(f"[REMAP DEBUG] Adding module/directory name 'main' to remap_map")
                    ...
                    if isinstance(node, ast.FunctionDef):
                        ...
                        if not is_external_func and node.name not in self.imports_whitelist and node.name != self.entry_function:
                            ...
                            if (not node.name.startswith('_imp_')
                                ...
                                print(f"[REMAP DEBUG] Adding function '{node.name}' to remap_map")
                                self.remap_map[node.name] = self.generate_random_name()
                        else:
                            print(f"[REMAP DEBUG] Skipping function '{node.name}' (is_external={is_external_func}, entry_function={node.name == self.entry_function})")
                            ...
                            if isinstance(target, ast.Name) and not target.id.startswith("_"):
                                ...
                                if (target.id not in self.imports_whitelist
                                    ...
                                    if target.id not in self.remap_map:
                                        if target.id == 'main':
                                            print(f"[REMAP DEBUG] Found assignment to 'main' in {py_file}")
                        ...
                        if isinstance(node.target, ast.Name) and not node.target.id.startswith("_"):
                            ...
                            if (node.target.id not in self.imports_whitelist
                                ...
                                if node.target.id not in self.remap_map:
                                    if node.target.id == 'main':
                                        print(f"[REMAP DEBUG] Found annotated assignment to 'main' in {py_file}")
                            ...
                            if alias.asname and not alias.asname.startswith("_"):
                                ...
                                if (alias.asname not in self.imports_whitelist
                                    ...
                                    if alias.asname not in self.remap_map:
                                        if alias.asname == 'main':
                                            print(f"[REMAP DEBUG] Found import alias 'main' in {py_file}")
                                ...
                                if module_name not in self.remap_map:
                                    if module_name == 'main':
                                        print(f"[REMAP DEBUG] Found import of module 'main' in {py_file}, keeping same name")
                        ...
                        for alias in node.names:
                            if alias.asname and not alias.asname.startswith("_"):
                                ...
                                if (alias.asname not in self.imports_whitelist
                                    ...
                                    if alias.asname not in self.remap_map:
                                        if alias.asname == 'main':
                                            print(f"[REMAP DEBUG] Found from-import alias 'main' in {py_file}")
                                        self.remap_map[alias.asname] = self.generate_random_name()
                            else:
